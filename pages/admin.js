...
export default function AdminDashboard() {
  ...
  const fetchCRMData = async () => {
    ...
    allProfiles.forEach(p => {
       ...
       merged.push({ ...p, ai_result: a ? a.interest_area : 'Pending' });
    });

    // Add assessments without profiles
    allAssess.forEach(a => {
  ...
  const fetchCMSData = async () => {
    const { data } = await supabase.from('matrix_content').select('*').order('created_at', { ascending: true });
  ...
  const handleCmsSave = async (e) => {
    ...
    if (formData.id) await supabase.from('matrix_content').update(payload).eq('id', formData.id);
    else await supabase.from('matrix_content').insert([payload]);
    setIsCmsSubmitting(false); setIsModalOpen(false); fetchCMSData();
  };
  const handleCmsDelete = async (id) => {
    if (window.confirm("Delete this matrix?")) { await supabase.from('matrix_content').delete().eq('id', id); fetchCMSData(); }
  };

  // QNA FETCH & SAVE
  const fetchQnaData = async () => {
    const { data } = await supabase.from('diagnostic_questions').select('*').order('created_at', { ascending: true });
  ...
  const handleQnaSave = async (e) => {
    ...
    if (qnaFormData.id) await supabase.from('diagnostic_questions').update(payload).eq('id', qnaFormData.id);
    else await supabase.from('diagnostic_questions').insert([payload]);
    setIsQnaSubmitting(false); setIsQnaModalOpen(false); fetchQnaData();
  };
  const toggleQuestionStatus = async (id, currentStatus) => {
    await supabase.from('diagnostic_questions').update({ is_active: !currentStatus }).eq('id', id); fetchQnaData();
  };
  ...
  return (
    <ErrorBoundary>
      <div style={{ backgroundColor: '#0f172a', minHeight: '100vh', color: '#f8fafc', padding: '40px 5%', fontFamily: "'Segoe UI', Roboto, sans-serif" }}>
            ...
            {adminTab === 'cms' && (
              <div style={{ background: 'rgba(30, 41, 59, 0.85)', padding: '30px', borderRadius: '16px', border: '1px solid rgba(56, 189, 248, 0.3)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '25px', flexWrap: 'wrap', gap: '15px' }}>
                      <div><h2 style={{ color: '#fff', margin: '0 0 5px 0' }}>Knowledge Bank Management</h2><p style={{ color: '#94a3b8', margin: 0, fontSize: '0.9rem' }}>Add, edit, or remove study domains and career streams.</p></div>
                  ...
                  <div>
                      {matrixContent.map(item => (
                          <div key={item.id} style={{ background: 'rgba(30, 41, 59, 0.6)', border: '1px dashed rgba(56, 189, 248, 0.3)', borderRadius: '10px', padding: '20px', marginBottom: '15px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                              <div>
                                  <h4 style={{ margin: '0 0 5px 0', color: '#fff', fontSize: '1.2rem' }}>{item.title_en} <span style={{fontSize:'0.8rem', color:'#38bdf8', background:'rgba(56,189,248,0.1)', padding:'2px 8px', borderRadius:'10px', marginLeft:'10px'}}>{item.stream_key}</span></h4>
                              </div>
              ...
              <div style={{ background: 'rgba(30, 41, 59, 0.85)', padding: '30px', borderRadius: '16px', border: '1px solid rgba(56, 189, 248, 0.3)' }}>
                  ...
                  <div>
                      {questionsData.map((q, i) => (
                          <div key={q.id} style={{ background: 'rgba(30, 41, 59, 0.6)', borderLeft: `4px solid ${q.is_active ? '#10b981' : '#ef4444'}`, borderRadius: '10px', padding: '20px', marginBottom: '15px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', opacity: q.is_active ? 1 : 0.5 }}>
                              <div style={{ flex: 1 }}>
        ...
        {/* DETAILED STUDENT MODAL (Now includes Profile Photo & AI Target) */}
        {selectedStudent && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 9999, padding: '20px', overflowY: 'auto' }}>
            <div style={{ background: '#1e293b', border: '1px solid #38bdf8', borderRadius: '16px', padding: '30px', width: '100%', maxWidth: '600px', position: 'relative' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', color: '#e2e8f0', marginBottom: '15px' }}>
                ...
                <div style={{ background: '#0f172a', padding: '15px', borderRadius: '8px', border: '1px solid #334155' }}>
                  <p style={{ margin: '0 0 5px 0', color: '#94a3b8', fontSize: '0.85rem' }}>AI Suggested Target</p><h3 style={{ margin: 0, fontSize: '1.1rem', color: '#10b981', textTransform: 'capitalize' }}>{selectedStudent.ai_result || 'Pending'}</h3>
                </div>
              </div>
        ...
        {/* CMS / QNA Modals (Minimized for space, but logic remains intact) */}
        {isModalOpen && (
