...
interface DBData {
  en: Record<string, CategoryData>;
  ur: Record<string, CategoryData>;
}
...
// --- BILINGUAL KNOWLEDGE BANK DATABASE (FALLBACK) ---
const fallbackDbData: DBData = {
  en: {
    science: { title: "Science & Technology Domain", scope: "Research, Advanced Data Science, Labs, Agriculture systems and professional engineering branches.", duration: "3 to 4 Years Degree Modules", items: ["Bsc Physics", "Bsc Chemistry", "Bsc Botany", "Bsc Zoology", "Bsc Computer science", "Bsc Mathematics", "Bsc PCM", "Bsc CBZ", "Bsc Forensic Science", "Bsc Food technology"], details: "" },
    commerce: { title: "Commerce & Strategic Finance Hub", scope: "Corporate accounting, banking, management systems, taxation laws and professional financial audits.", duration: "3 Years Standard Graduation Route", items: ["CA Chartered Account", "CMA Cost Management Account", "CS Company Secretary", "B.Com Regular", "B.Com Taxation", "BBA / BBM Regular", "BFM Financial Management"], details: "" },
    paramedical: { title: "Paramedical & Healthcare Allied Science", scope: "Clinical pharmacy solutions, pathology laboratory expertise, radiology metrics, medical scanning and nursing fields.", duration: "2 to 4 Years (Diploma / Degrees)", items: ["Nursing", "Pharm D", "B.Pharm", "D.Pharm", "Anesthesia technical", "Cardiac Care technical", "Clinical Optometry", "Medical Lab technician", "PHYSIOTHERAPY"], details: "" },
    btech: { title: "Advanced Engineering Framework", scope: "Software development pipelines, robotic architectures, automation engineering, infrastructure mapping, system configurations.", duration: "4 Years Professional Engineering Degree", items: ["Computer Science Engi.", "Electronics & Comm.Engi.", "Mechanical Engineering", "Civil Engineering", "Automation & Robotics Eng.", "Biomedical Engineering"], details: "" }
  },
  ur: {
    science: { title: "سائنس اور ٹیکنالوجی ڈومین", scope: "ریسرچ، ایڈوانسڈ ڈیٹا سائنس، لیبز، ایگریکلچر سسٹمز اور پروفیشنل انجینئرنگ برانچز۔", duration: "3 سے 4 سالہ ڈگری ماڈیولز", items: ["Bsc Physics", "Bsc Chemistry", "Bsc Botany", "Bsc Zoology", "Bsc Computer science", "Bsc Mathematics", "Bsc PCM", "Bsc CBZ", "Bsc Forensic Science", "Bsc Food technology"], details: "" },
    ...
    btech: { title: "ایڈوانسڈ انجینئرنگ فریم ورک", scope: "سافٹ ویئر ڈیولپمنٹ، روبوٹک آرکیٹیکچر، آٹومیشن انجینئرنگ، انفراسٹرکچر میپنگ، SYSTEM کنفیگریشنز۔", duration: "4 سالہ پروفیشنل انجینئرنگ ڈگری", items: ["Computer Science Engi.", "Electronics & Comm.Engi.", "Mechanical Engineering", "Civil Engineering", "Automation & Robotics Eng.", "Biomedical Engineering"], details: "" }
  }
};
...
const t: Record<Lang, Record<string, string>> = {
  en: {
    ...
    myProfile: "My Profile", logout: "Logout"
  },
  ur: {
    ...
    myProfile: "میری پروفائل", logout: "لاگ آؤٹ"
  }
};

// --- ELITE ISR CACHING (RULE 5) ---
export async function getStaticProps() {
  try {
    const { data, error } = await supabase.from('matrix_content').select('*');
    if (error) throw error;

    let serverDbData: DBData = { en: {}, ur: {} };
    if (data && data.length > 0) {
      data.forEach(item => {
        serverDbData.en[item.stream_key] = { 
          title: item.title_en, scope: item.scope_en, duration: item.duration_en, items: item.courses_en || [], details: item.details_en || '' 
        };
        serverDbData.ur[item.stream_key] = { 
          title: item.title_ur, scope: item.scope_ur, duration: item.duration_ur, items: item.courses_ur || [], details: item.details_ur || '' 
        };
      });
    } else {
      serverDbData = fallbackDbData; // Fallback if CMS is empty
    }

    return {
      props: { initialDbData: serverDbData },
      revalidate: 60, // Refetches from DB every 60 seconds (Cache Mastery)
    };
  } catch (err) {
    return {
      props: { initialDbData: fallbackDbData },
      revalidate: 60,
    };
  }
}

export default function CourseCategories({ initialDbData }: CategoriesProps) {
  ...
  const [dbData] = useState<DBData>(initialDbData); // Loaded instantly from ISR Cache
  ...
  useEffect(() => {
    if (stream && typeof stream === 'string' && dbData.en[stream]) {
        setActiveTab(stream);
      ...
      <nav className="glass-navbar">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
            ...
            {session ? (
              <>
                ...
                <button onClick={handleLogout} className="auth-icon-btn logout-btn" title={t[lang].logout}><i className='bx bx-log-out'></i></button>
              </>
            ) : (
        ...
        <div className="desktop-menu">
          ...
          <div className="nav-dropdown-container" onMouseEnter={() => !isMobile && setShowGuidanceDropdown(true)} onMouseLeave={() => !isMobile && setShowGuidanceDropdown(false)}>
            <button className="nav-link" onClick={() => isMobile && setShowGuidanceDropdown(!showGuidanceDropdown)} style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>{t[lang].navCareer} <i className='bx bx-chevron-down'></i></button>
